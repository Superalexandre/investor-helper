...
export const APIRoute = createAPIFileRoute("/api/data/prices/multiples")({
	GET: async ({ request }) => {
		...
		const pricePromises = symbols.map(async (symbol) => {
			...
			for (const [, allPrice] of allPrices.entries()) {
				const price = priceResult.period.find(
					(p) => Math.abs(allPrice.date.getTime() - p.time * 1000) < timeframe.threshold
				)
				if (price) {
