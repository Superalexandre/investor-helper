...
export function useSWEffect(options: UseSWEffectOptions = { cacheType: 'jit' }): void {
  ...
  useEffect(() => {
    const sendMessageToSW = (worker: ServiceWorker, message: { type: string; payload: SWMessagePayload }) => {
        worker.postMessage(message);
      };
    // const sendMessageToSW = (event: string, payload: SWMessagePayload) => {
    //   if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
    //     worker.postMessage(navigator.serviceWorker.controller, { type: event, payload });
    //   }
    // };
    ...
    const handleLocationChange = () => {
      ...
      messageDebounceRef.current = setTimeout(() => {
        ...
        if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
            sendMessageToSW(navigator.serviceWorker.controller, { type: event, payload });
          }
      }, 25);
    };
